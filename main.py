...
class AdvancedGraphRAGSystem:
    ...
    def _show_knowledge_base_stats(self):
        ...
        print(f"   疾病数量: {stats.get('total_diseases', 0)}")
        print(f"   症状数量: {stats.get('total_symptoms', 0)}")
        print(f"   治疗方案: {stats.get('total_treatments', 0)}")
        print(f"   药物数量: {stats.get('total_medications', 0)}")
        ...
        if stats.get('categories'):
            ...
            print(f"   🏷️ 主要疾病分类: {', '.join(categories)}")
        if stats.get('severity_distribution'):
            severities = list(stats['severity_distribution'].items())
            top_severities = ', '.join(f"{k}:{v}" for k, v in severities[:5])
            if top_severities:
                print(f"   ⚕️ 严重程度分布: {top_severities}")
        ...
        try:
            ...
            if relevant_docs:
                ...
                for doc in relevant_docs:
                    entity_name = doc.metadata.get('entity_name', doc.metadata.get('name', '未知实体'))
