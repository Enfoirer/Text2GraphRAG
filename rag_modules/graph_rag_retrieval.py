...
class GraphRAGRetrieval:
    ...
    def understand_graph_query(self, query: str) -> GraphQuery:
        ...
        请识别：
        1. 查询类型：
           - entity_relation: 询问实体间的直接关系（如：白内障与糖尿病的关系是什么？）
           - multi_hop: 需要多跳推理（如：糖尿病如何导致视网膜水肿？需要：糖尿病→血管损伤→视网膜水肿）
           - subgraph: 需要完整子图（如：干眼症相关的症状、用药和护理？）
           - path_finding: 路径查找（如：慢性炎症如何引发青光眼？）
           - clustering: 聚类相似性（如：哪些疾病表现与黄斑变性相似？）
        ...
        示例：
        查询："糖尿病视网膜病变常见哪些症状？"
        分析：这是multi_hop查询，需要通过"糖尿病视网膜病变→症状节点"的路径推理
        
        返回JSON格式：
        {{
            "query_type": "multi_hop",
            "source_entities": ["糖尿病视网膜病变"],
            "target_entities": ["视网膜症状"],
            "relation_types": ["HAS_SYMPTOM"],
            "max_depth": 2,
            "reasoning": "需要多跳推理：疾病→症状节点"
        }}
        """
